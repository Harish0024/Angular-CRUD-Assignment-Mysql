...
function updatecontact(request,response){
    ...
    var phonenumber=request.body.number;
    mssqlconnection.connect(checkConnection);
    mssqlconnection.query("update contactus set lastname=?,emailid=?,phonenumber=? where firstname=?",[lastname,emailid,phonenumber,firstname],processResults);
    response.send(statusMessage);
}
...
function Deleteuser(request,response){
    ...
    mssqlconnection.connect(checkConnection);
    mssqlconnection.query("delete from contactus where firstname=?",[firstname],processResults);
    response.send(statusMessage);
}
